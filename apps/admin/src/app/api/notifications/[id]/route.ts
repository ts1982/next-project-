...
export const PATCH = withApiHandler(
  async (request, { params }) => {
    ...
    try {
      ...
      return NextResponse.json(successResponse(result, "通知を更新しました"));
    } catch (err) {
      if (err instanceof ConflictError) {
        return NextResponse.json(errorResponse(err.message, undefined, "ALREADY_DELIVERED"), {
          status: 409,
        });
      }
      throw err;
...
export const DELETE = withApiHandler(
  async (request, { params }) => {
    ...
    try {
      ...
      return NextResponse.json(successResponse(null, "通知を削除しました"));
    } catch (err) {
      if (err instanceof ConflictError) {
        return NextResponse.json(errorResponse(err.message, undefined, "ALREADY_DELIVERED"), {
          status: 409,
