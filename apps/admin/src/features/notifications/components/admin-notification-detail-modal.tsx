        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {notification.deliveredAt && (
              <div>
                <p className="text-xs text-muted-foreground">配信日時</p>
                <p className="text-sm">
            ...
            {notification.targetType === "SPECIFIC" && notification.targets.length > 0 && (
              <div>
                <p className="text-xs text-muted-foreground">送信先ユーザー</p>
                <div className="flex flex-wrap gap-1 mt-1">
                  {notification.targets.map((t) => (
                    <span key={t.id} className="text-xs px-2 py-0.5 rounded-full bg-muted">
                      {t.user.name}
                    </span>
                  ))}
                </div>
