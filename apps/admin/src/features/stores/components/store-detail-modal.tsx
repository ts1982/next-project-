...
export function StoreDetailModal({ store, isOpen, onClose, timezone }: StoreDetailModalProps) {
  ...
  const initializeEditForm = useCallback(() => {
    ...
    setPublishedAt(store.publishedAt ? toZonedTime(store.publishedAt, timezone) : undefined);
    setUnpublishedAt(store.unpublishedAt ? toZonedTime(store.unpublishedAt, timezone) : undefined);
    setErrors({});
  }, [store, timezone]);
                  ...
                  {store.publishedAt ? (
                    <div className="text-sm mt-1">
                      ...
                      <p>
                        終了:{" "}
                        {store.unpublishedAt
                          ? formatDateTime(store.unpublishedAt, timezone)
                          : "無期限"}
                      </p>
                    </div>
                  ) : (
