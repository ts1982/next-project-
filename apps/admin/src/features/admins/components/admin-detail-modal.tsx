...
import { updateAdminSchema, type UpdateAdminInput } from "../schemas/admin.schema";
...
export function AdminDetailModal({ admin, isOpen, onClose, timezone }: AdminDetailModalProps) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState("detail");
  const [formData, setFormData] = useState<Partial<UpdateAdminInput>>({});
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    if (formData.roleId && formData.roleId !== admin.roleId) updateData.roleId = formData.roleId;
  ...
  return (
    <>
            ...
            <TabsContent value="detail" className="space-y-4 mt-4">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  <div className="mt-1">
                    <RoleBadge roleName={admin.role.name} />
                  </div>
                </div>

                <div className="pt-2 border-t">
                  <p className="text-xs text-muted-foreground">
                    作成日: {formatDateTime(admin.createdAt, timezone)}
                  </p>
                  <p className="text-xs text-muted-foreground">
                    更新日: {formatDateTime(admin.updatedAt, timezone)}
                  </p>
                </div>
              </div>
            ...
            <TabsContent value="edit" className="mt-4">
              <form onSubmit={handleEdit} className="space-y-4">
                ...
                <div>
                  ...
                  <RoleSelect
                    id="edit-role"
                    value={formData.roleId || admin.roleId}
      ...
      <ConfirmDialog
        ...
        title="削除の確認"
        description={`「${admin.name}」を削除してもよろしいですか？この操作は取り消せません。`}
        confirmLabel="削除"
        variant="destructive"
